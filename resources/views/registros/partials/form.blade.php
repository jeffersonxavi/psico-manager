<div x-data="registroForm()" class="p-4 sm:p-6">
    <form id="registroForm" action="{{ route('registros.store', [$sessao->paciente_id, $sessao->id]) }}" method="POST"
        class="space-y-4"> {{-- Espaçamento ligeiramente menor (space-y-4) --}}
        @csrf
        <input type="hidden" name="sessao_id" value="{{ $sessao->id }}">

        {{-- ## 1. Seleção de tipo (Mais Compacto) --}}
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2"> {{-- Gap menor (gap-2) --}}
            @foreach(\App\Models\Registro::$tipos as $t)
                <label class="cursor-pointer group">
                    ...
                    <div @click="tipo = '{{ $t }}'; $nextTick(() => $refs.texto.focus())"
                        ...
                        class="flex flex-col items-center justify-center gap-2 p-3 sm:p-4 rounded-xl border-2 transition-all duration-300 transform hover:scale-[1.01] hover:shadow-md">
                        {{-- Padding reduzido e hover mais sutil --}}
                        <span x-html="icone('{{ $t }}')" class="w-6 h-6 sm:w-7 sm:h-7"></span> {{-- Ícones ligeiramente
                        menores --}}
                        <span class="text-xs font-semibold text-center">{{ ucfirst(str_replace('_', ' ', $t)) }}</span>
                    </div>
                </label>
            @endforeach
        </div>
        ...
        <div x-transition x-show="tipo">
            <textarea x-ref="texto" name="conteudo" rows="3" required {{-- Linhas reduzidas para 3 --}}
                placeholder="Descreva o registro da sessão..."
                class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 text-sm resize-none transition-all duration-300 placeholder-gray-400"></textarea>
        </div>

        {{-- ## 3. Botão --}}
        <button type="submit" x-show="tipo"
            class="w-full py-3 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold text-base shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-3">
...
<!-- SweetAlert2 | Global Loading -->
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>

<script>
    document.addEventListener("DOMContentLoaded", () => {

        // 🔥 Loading global no submit
